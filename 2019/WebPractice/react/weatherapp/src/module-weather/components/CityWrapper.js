...
export class CityWrapper extends React.Component {
  state = { modalDisplay: false, autoCompData: "", cityItems: [] };

  modalDisplay = () =>
    this.setState(state => ({ modalDisplay: !state.modalDisplay }));
  //Modal data
  onSubmit = async () => {
    let cityname = this.state.autoCompData;
    try {
      const response = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?q=${cityname}&APPID=2f8dd18e1f11a92f2feaf7c73623413c&units=metric`
      );
  ...
  itemArrCond = () => {
    return this.state.cityItems.map(
      ({ name, main: { temp_min, temp_max, temp }, ...data }) => {
